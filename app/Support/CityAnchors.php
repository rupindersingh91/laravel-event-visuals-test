...
class CityAnchors
{
    /** @var array<int, array{0: float, 1: float, 2: string}> */
    ...
    public static function nearest(float $lat, float $lng): string
    {
        $best = 'Unknown';
    ...
    /** @return array{lat_min: float, lat_max: float, lng_min: float, lng_max: float}|null */
    public static function boundingBox(string $cityLabel, float $pad = 0.6): ?array
    {
        foreach (self::ANCHORS as $anchor) {
            if ($anchor[2] === $cityLabel) {
                return [
                    'lat_min' => $anchor[0] - $pad,
                    'lat_max' => $anchor[0] + $pad,
                    'lng_min' => $anchor[1] - $pad,
                    'lng_max' => $anchor[1] + $pad,
                ];
            }
        }
    ...
    {
        return array_column(self::ANCHORS, 2);
    }

    /** @return array<string, array{0: float, 1: float}> */
    public static function coordsByLabel(): array
    {
        $result = [];
        foreach (self::ANCHORS as $anchor) {
            $result[$anchor[2]] = [$anchor[0], $anchor[1]];
        }
        return $result;
    }
}
